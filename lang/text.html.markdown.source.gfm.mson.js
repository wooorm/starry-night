...
const grammar = {
  extensions: [],
  names: [],
  patterns: [{include: '#mson-field'}, {include: '#gfm'}],
  repository: {
    gfm: {
      patterns: [
    ...
    'mson-example-value': {
      captures: {1: {name: 'mson.field.example markup.raw.inline'}},
      match: ':\\s(`.*?`|(.*?)(?=\\(|-|$))'
    },
    'mson-field': {
      patterns: [
    ...
    'mson-field-attributes': {
      ...
      patterns: [
        {
          match:
            '(required|optional|variable|fixed(-type)?|sample|default|nullable)',
          name: 'keyword'
        },
        {
