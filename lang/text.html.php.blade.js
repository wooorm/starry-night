...
const grammar = {
  ...
  repository: {
    balance_brackets: {
      patterns: [
        {begin: '\\(', end: '\\)', patterns: [{include: '#balance_brackets'}]},
        {match: '[^()]+'}
      ]
    },
    blade: {
      patterns: [
        {
          begin: '{{--',
          beginCaptures: {
            0: {name: 'punctuation.definition.comment.begin.blade'}
          },
          end: '--}}',
        ...
        {match: '@(?={{{|{{|{!!|@\\w+(?:::\\w+)?)', name: 'comment.blade'},
        {
          ...
          endCaptures: {
            0: {name: 'support.function.construct.end.blade'},
            1: {name: 'source.php'}
          },
        ...
        },
        {
          ...
          beginCaptures: {0: {name: 'keyword.begin.blade'}},
          contentName: 'source.php',
          end: '(?<![A-Za-z0-9_@])(?=@(?i:endphp)\\b)',
          endCaptures: {0: {name: 'keyword.end.blade'}},
      ...
      patterns: [
        {
          ...
          endCaptures: {
            0: {name: 'punctuation.definition.arguments.end.bracket.round.php'}
          },
        ...
        {
          begin:
            '(?i)(\\\\)?\\b([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)\\s*(\\()',
          beginCaptures: {
    ...
    'function-parameters': {
      patterns: [
        ...
        {match: ',', name: 'punctuation.separator.delimiter.php'},
        {
          ...
          endCaptures: {
            0: {name: 'punctuation.definition.array.end.bracket.round.php'}
          },
    ...
    heredoc: {
      patterns: [
        {
          begin:
            '(?i)(?=<<<\\s*("?)([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)(\\1)\\s*$)',
          end: '(?!\\G)',
      ...
      patterns: [
        ...
        },
        {
          ...
          patterns: [
            ...
            {
              begin:
                '(?i)(?<=^|\\s)(#)\\s(?=[[a-z0-9_\\x{7f}-\\x{ff},. \\t?!-][^\\x{00}-\\x{7f}]]*$)',
        ...
        {
          begin:
            '(?i)(<<<)\\s*("?)([a-z_\\x{7f}-\\x{ff}]+[a-z0-9_\\x{7f}-\\x{ff}]*)(\\2)(\\s*)',
          beginCaptures: {
            1: {name: 'punctuation.definition.string.php'},
            3: {name: 'keyword.operator.heredoc.php'},
            5: {name: 'invalid.illegal.trailing-whitespace.php'}
    ...
    'invoke-call': {
      ...
      match:
        '(?i)(\\$+)([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)(?=\\s*\\()',
    ...
    language: {
      patterns: [
        ...
        {
          begin:
            '(?i)^\\s*(interface)\\s+([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)\\s*(extends)?\\s*',
          beginCaptures: {
        ...
        {
          begin:
            '(?i)^\\s*(trait)\\s+([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)',
          beginCaptures: {
        ...
        },
        {
          ...
          match:
            '(?i)(?:^|(?<=<\\?php))\\s*(namespace)\\s+([a-z0-9_\\x{7f}-\\x{ff}\\\\]+)(?=\\s*;)',
        ...
        {match: '\\s+(?=use\\b)'},
        {
          ...
          patterns: [
            ...
            },
            {
              begin: '{',
              beginCaptures: {
                0: {name: 'punctuation.definition.use.begin.bracket.curly.php'}
              },
              end: '}',
              endCaptures: {
                0: {name: 'punctuation.definition.use.end.bracket.curly.php'}
              },
              patterns: [
                ...
                {
                  captures: {
                    1: {name: 'keyword.other.use-insteadof.php'},
                    2: {name: 'support.class.php'}
                  },
                  match:
                    '(?i)\\b(insteadof)\\s+([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)'
                },
        ...
        {
          begin:
            '(?i)^\\s*(?:(abstract|final)\\s+)?(class)\\s+([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)',
          beginCaptures: {
          ...
          endCaptures: {
            0: {name: 'punctuation.definition.class.end.bracket.curly.php'}
          },
          name: 'meta.class.php',
          patterns: [
            {include: '#comments'},
              ...
              patterns: [
                {
                  ...
                  match:
                    '(?i)((&)?\\s*(\\$+)[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)\\s*(?=,|\\))',
        ...
        {include: '#instantiation'},
        {
          ...
          match:
            '(?i)(goto)\\s+([a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)'
        },
        {
        ...
        {include: '#string-backtick'},
        {
          begin: '{',
          beginCaptures: {
            0: {name: 'punctuation.definition.begin.bracket.curly.php'}
          },
          end: '}|(?=\\?>)',
          endCaptures: {
            0: {name: 'punctuation.definition.end.bracket.curly.php'}
          },
          patterns: [{include: '#language'}]
        },
        {
        ...
        },
        {
          begin: '\\(',
          beginCaptures: {
            0: {name: 'punctuation.definition.begin.bracket.round.php'}
          },
          end: '\\)|(?=\\?>)',
          endCaptures: {
            0: {name: 'punctuation.definition.end.bracket.round.php'}
          },
          patterns: [{include: '#language'}]
        },
        {include: '#constants'},
    ...
    php_doc_types_array_multiple: {
      begin: '\\(',
      beginCaptures: {
        0: {name: 'punctuation.definition.type.begin.bracket.round.phpdoc.php'}
      },
      end: '(\\))(\\[\\])|(?=\\*/)',
      endCaptures: {
    ...
    php_doc_types_array_single: {
      captures: {
        1: {patterns: [{include: '#php_doc_types'}]},
        2: {name: 'keyword.other.array.phpdoc.php'}
      },
      match:
        '(?i)([a-z_\\x{7f}-\\x{ff}\\\\][a-z0-9_\\x{7f}-\\x{ff}\\\\]*)(\\[\\])'
    ...
    'scope-resolution': {
      patterns: [
        ...
        },
        {
          captures: {
            1: {name: 'keyword.operator.class.php'},
            2: {name: 'keyword.other.class.php'}
          },
          match: '(?i)(::)\\s*(class)\\b'
        },
        {
    ...
    'sql-string-double-quoted': {
      begin:
        '"\\s*(?=(SELECT|INSERT|UPDATE|DELETE|CREATE|REPLACE|ALTER|AND)\\b)',
      beginCaptures: {0: {name: 'punctuation.definition.string.begin.php'}},
    ...
    support: {
      patterns: [
        ...
        {
          match:
            '(?i)\\bfilter_(has_var|input(_array)?|id|var(_array)?|list)\\b',
    ...
    'variable-name': {
      patterns: [
        ...
        },
        {
          ...
          match:
            '(?i)((\\${)(?<name>[a-z_\\x{7f}-\\x{ff}][a-z0-9_\\x{7f}-\\x{ff}]*)(}))'
        }
      ]
    },
    variables: {
