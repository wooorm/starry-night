...
// <https://github.com/inferrna/lobster_ling>
...
const grammar = {
  ...
  repository: {
    comments: {
      patterns: [
        {begin: '/\\*', end: '\\*/', name: 'comment.block.source.lobster'},
        {match: '//.*$', name: 'comment.line.double-slash.lobster'}
      ]
    },
    digits: {
      patterns: [
        ...
        {match: '(\\d+)?\\.(\\d+)?', name: 'constant.numeric.float.lobster'}
      ]
    },
    ident: {
      patterns: [
        {match: '[A-Za-z_][A-Za-z0-9_]*', name: 'variable.source.lobster'}
      ]
    },
    keywords: {
      patterns: [
        {
          match:
            '\\b(constructor|abstract|member|member_frame|static|static_frame|attribute|break|return|struct|class|import|namespace|pakfile|def|is|from|program|private|enum|enum_flags|typeof|var|let|not|and|or)\\b',
    ...
    statements: {
      patterns: [
        {
          begin: '\\b(let|var)\\s+',
          beginCaptures: {1: {name: 'storage.type.lobster'}},
          end: '=',
          endCaptures: {0: {name: 'punctuation.definition.lobster'}},
          name: 'variable.assignment.lobster',
          patterns: [
            {match: '[a-zA-Z_][a-zA-Z0-9_]*', name: 'variable.name.lobster'},
            {match: ',', name: 'punctuation.separator.arguments.lobster'}
          ]
        },
        {
          begin: '\\b(def|constructor)\\s+([A-Za-z_][A-Za-z0-9_]*)(\\()',
          beginCaptures: {
            1: {name: 'keyword.control.lobster'},
            2: {name: 'entity.name.function.lobster'},
            3: {name: 'punctuation.definition.parameters.begin.lobster'}
          },
          end: '\\)(?:\\s*->\\s*)?((\\[?[A-Za-z_][A-Za-z0-9_]*[A-Za-z_]*\\]?\\??(, ?)?)*?):',
          endCaptures: {
            0: {name: 'punctuation.definition.parameters.end.lobster'},
            1: {name: 'storage.type.source.lobster'}
          },
          ...
          patterns: [
            ...
            {
              captures: {1: {name: 'storage.type.source.lobster'}},
              match:
                '(?<![:]):{,2}(int|float|string|resource|any|void|[A-Za-z_][A-Za-z0-9_]*)'
            },
            {match: ',', name: 'punctuation.separator.arguments.lobster'}
          ]
        },
        {
          ...
          beginCaptures: {
            1: {name: 'support.class.lobster'},
            2: {name: 'entity.name.class.lobster'}
          },
          end: ':',
        ...
        },
        {
          ...
          beginCaptures: {
            1: {name: 'support.modifier.lobster'},
            2: {name: 'entity.name.type.enum.lobster'}
          },
          end: ':',
          endCaptures: {0: {name: 'punctuation.section.enum.end.lobster'}},
    ...
    strings: {
      patterns: [
        {begin: '"{3}', end: '"{3}', name: 'string.quoted.triple.lobster'},
        {begin: '"', end: '"', name: 'string.quoted.double.lobster'},
        {
          match:
            "'(\\\\n|\\\\t|\\\\r|\\\\\"|\\\\'|\\\\x[[:xdigit:]]{2}|\\w|\\\\|)'",
