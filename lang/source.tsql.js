...
// <https://github.com/beau-witter/language-tsql>
// and licensed `mit`.
...
const grammar = {
  ...
  patterns: [
    ...
    {
      match:
        '(?i:\\b(select(\\s+distinct)?|insert\\s+(ignore\\s+)?into|update|delete|from|set|where|group\\s+by|or|like|and|union(\\s+all)?|having|order\\s+by|limit|(inner|cross)\\s+join|join|straight_join|full\\s+outer\\s+join|(left|right)(\\s+outer)?\\s+join|natural(\\s+(left|right)(\\s+outer)?)?\\s+join)\\b)',
    ...
    {match: '(?i)\\b(cursor_status)\\b', name: 'support.function.cursor.sql'},
    {
      match:
    ...
    {
      match:
        '(?i)\\b(ascii|char|charindex|concat|difference|format|left|len|lower|ltrim|nchar|nodes|patindex|quotename|replace|replicate|reverse|right|rtrim|soundex|space|str|string_agg|string_escape|string_split|stuff|substring|translate|trim|unicode|upper)\\b',
