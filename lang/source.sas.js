...
// <https://github.com/rpardee/sas.tmbundle>
// and licensed `mit`.
...
const grammar = {
  ...
  patterns: [
    ...
    {include: '#operator'},
    {
      ...
      patterns: [
        ...
        {include: '#dataSet'},
        {
          ...
          match:
            '(?i:(?:(stack|pgm|view|source)\\s?=\\s?)|(debug|nesting|nolist))'
        }
      ]
    },
    {
    ...
    {
      match:
        '(?i:\\b(if|while|until|for|do|end|then|else|run|quit|cancel|options)\\b)',
  ...
  repository: {
    blockComment: {
      patterns: [
        {begin: '\\/\\*', end: '\\*\\/', name: 'comment.block.slashstar.sas'}
      ]
    },
    constant: {
      patterns: [
    ...
    operator: {
      patterns: [
        {match: '([\\+\\-\\*\\^\\/])', name: 'keyword.operator.arithmetic.sas'},
        {
    ...
    quote: {
      patterns: [
        ...
        {begin: '(")', end: '(")([bx])?', name: 'string.quoted.double.sas'}
      ]
    },
    starComment: {
      patterns: [
