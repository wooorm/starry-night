...
const grammar = {
  ...
  patterns: [
    ...
    {
      begin:
        "^(\\s*)(foreign)\\s+(import)\\s+(data)\\s+([\\p{Lu}\\p{Lt}][\\p{Ll}_\\p{Lu}\\p{Lt}\\p{Nd}']*)",
      beginCaptures: {
        2: {name: 'keyword.other.purescript'},
        3: {name: 'keyword.other.purescript'},
        4: {name: 'keyword.other.purescript'},
    ...
    },
    {
      ...
      patterns: [
        ...
        {
          captures: {0: {name: 'punctuation.separator.pipe.purescript'}},
          match: '\\|'
        },
    ...
    {
      match:
        "^\\s*\\b(derive|where|data|type|newtype|infix[lr]?|foreign(\\s+import)?(\\s+data)?)(?!')\\b",
