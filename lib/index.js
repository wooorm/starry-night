...
export async function createStarryNight(grammars, options) {
  ...
  function flagToScope(flag) {
    ...
    while (start < end) {
      const code = flag.codePointAt(start)
    ...
    while (end > start) {
      const code = flag.codePointAt(end - 1)

      if (code !== 9 && code !== 32 && code !== 47 /* `/` */) {
        break
      }
    ...
    const normal = flag.slice(start, end).toLowerCase()
  ...
  /**
   * Create a registry with the given grammars.
   *
   * @param {ReadonlyArray<Readonly<Grammar>>} grammars
   *   Grammars to support.
   * @param {Readonly<Options> | null | undefined} [options]
   *   Configuration (optional).
   * @returns
   *   Registry with the given grammars.
   */
  async function createRegistry(grammars, options) {
    for (const grammar of grammars) {
      const scope = grammar.scopeName
      for (const d of grammar.extensions) {
        extensions.set(d, scope)
      }
    ...
    await Promise.all(
      [...registered.keys()].map((d) => registry.loadGrammar(d))
    )
