...
/**
 * Delve into a node if there’s a class name, creating a new element if needed.
 *
 ...
 *   New element, if with `className`, or the given `scope`.
 */
...
/**
 * Append text to a node, creating a text node if needed.
 *
 * @param {Element | Root} scope
 *   Node to work in.
 * @param {string} value
...
function colorToClass(color) {
  ...
  return classes[Math.trunc(Number(color.slice(1)))]
}
